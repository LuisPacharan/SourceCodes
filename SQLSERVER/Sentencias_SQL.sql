...
/*---------- DECLARACION DE VARIABLES (DECLARE - SET) -------------*/

DECLARE @ANIO VARCHAR(10),@MES VARCHAR(10),@DIA VARCHAR(10)
SET @ANIO = DATEPART(YY,GETDATE()-1) 
SET @MES  = DATEPART(MM,GETDATE()-1) 
SET @DIA  = DATEPART(DD,GETDATE()-1) 
...
'  SELECT * FROM TBL_FACTURAS
    WHERE YEAR(FECHA_FACT)  = '+@ANIO+'
     AND   MONTH(FECHA_FACT) = '+@MES+'
      AND   DAY(FECHA_FACT)   =  '+@DIA+'
'
